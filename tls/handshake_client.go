...
func makeClientHello(config *Config) (*clientHelloMsg, error) {
	if len(config.ServerName) == 0 && !config.InsecureSkipVerify && len(config.ESNIServerName) == 0 {
		return nil, errors.New("tls: either ServerName, ESNIServerName, or InsecureSkipVerify must be specified in the tls.Config")
	}
...
func (c *Conn) clientHandshake() error {
	...
	if c.config.maxVersion() >= VersionTLS13 {
		...
		if c.config.ClientESNIKeys != nil {
			...
			esniNonce, esniExt, err := esniKeys.makeClientHelloExtension(c.config.rand(), c.config.ESNIServerName, hello.random, hello.keyShares)
			if esniExt == nil {
				// Bad serverName, etc.
				return fmt.Errorf("tls: invalid ESNI configuration: %s", err)
			}
			hello.encryptedServerName = esniExt.marshal()
			if !c.config.PreserveSNI {
