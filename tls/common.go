...
type Config struct {
	...
	// ESNIServerName is used in the ESNI TLS extention to determine
	// the actual host to connect to. It does not need to match
	// the SNI header, and providers my allow both an ESNI and SNI
	// header. Cloudflare allows both, but only cares about the ESNI header
	ESNIServerName string

	// preserveSNI indicates whether or not to maintain the encrypted
	// SNI header if an ESNI is used, defaults to false
	PreserveSNI bool
}
...
func (c *Config) Clone() *Config {
	...
	return &Config{
		...
		ESNIServerName:              c.ESNIServerName,
		PreserveSNI:                 c.PreserveSNI,
	}
}
